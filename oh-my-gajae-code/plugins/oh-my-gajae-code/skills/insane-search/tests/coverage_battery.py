...
def main(argv: list[str]) -> int:
    ...
    strict_routes = "--strict-routes" in argv
    wanted = [a for a in argv if not a.startswith("-")]
    unknown = sorted(set(wanted) - set(BATTERIES))
    if unknown:
    ...
    any_route_fail = any(not result.ok for result in results)
    return 1 if any_total_fail or (strict_routes and any_route_fail) else 0
