...
class FetchResult:
    ...
    must_invoke_browser: bool = False
    content_trust: str = ""
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "must_invoke_browser": self.must_invoke_browser,
            "content_trust": self.content_trust,
            "prompt_injection_risk": self.prompt_injection_risk,
        ...
        try:
            ...
            for fb_name in fb_order:
                if fb_name == "curl_grid_exhaust":
                    continue
                # A "gjc_browser" entry can only be driven from the agent
                ...
                is_mcp_stub = fb_name.startswith("gjc_browser")
...
def _untried_routes(stop_reason, grid_exhausted) -> tuple[list[str], bool]:
    ...
    Returns (untried_routes, must_invoke_browser).
    """
    routes: list[str] = []
    ...
    if rate_limited:
        routes.append("rate-limited (429) — transient: back off a few seconds, then retry once. Do NOT hammer the grid.")
    ...
    # The hardened OMG port has no browser escape hatch: browser navigation
    # cannot preserve the DNS-pinning contract for every redirect/subresource.
    must_browser = False
    routes.append("device retry: re-run the CLI once with --device mobile")
    return routes, must_browser
...
def _give_up(trace, profile_used, last_resp, last_attempt, best_suspect,
    ...
    untried, must_browser = _untried_routes(stop_reason, grid_exhausted)
