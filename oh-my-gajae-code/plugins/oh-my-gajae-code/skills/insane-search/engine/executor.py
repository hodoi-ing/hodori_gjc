...
def _auto_install(pkg: str) -> bool:
    # Runtime package installation is intentionally disabled in OMG. Fetching
    # public content must not execute unpinned installer code or mutate the
    # user's active Python environment.
    return False
...
def _run_protocol_stealth(
    ...
    att.elapsed_s = round(time.time() - t0, 3)
    if not att.error:
        att.error = "nodriver/patchright not installed; runtime installation is disabled"
...
def run_playwright_fallback(
    ...
    *,
    ...
    "playwright_mobile_chrome", "gjc_browser".

    Returns (Attempt, html_content). Attempt.verdict reflects validation.
    """
    from . import safety
    safe, reason = safety.classify_url(url)
    if not safe:
        att = Attempt(
            phase="fallback", executor="blocked", url=url,
            url_transform="original", impersonate=None, referer="",
            error=f"ssrf_blocked:{reason}", verdict=Verdict.UNKNOWN.value,
        )
        return att, ""
    ...
    if choice.startswith("gjc_browser"):
        att.error = (
            "Browser fallback must be invoked explicitly through GJC's browser tool; "
            "the local fetch engine does not automate a browser."
        )
        att.verdict = Verdict.UNKNOWN.value
        att.elapsed_s = round(time.time() - t0, 3)
        return att, ""
    ...
    selected_profile_dir = profile_dir or _profile_dir_for(url, choice)
    os.makedirs(selected_profile_dir, mode=0o700, exist_ok=True)
    os.chmod(selected_profile_dir, 0o700)
    args: dict = {
        "url": url,
        ...
        "profileDir": selected_profile_dir,
    ...
    safe, reason = safety.classify_url(final_url or url)
    if not safe:
        att.error = f"ssrf_redirect_blocked:{reason}"
        att.verdict = Verdict.UNKNOWN.value
        return att, ""
